...
print(away_team_impact.isnull().sum())
away_team_impact = away_team_impact[['away_team', 'attendance_zscore', 'away_team_impact','division']].sort_values('attendance_zscore', ascending=False).reset_index()
print(away_team_impact.head(50))


sns.barplot(data = away_team_impact.head(30).sort_values(['division', 'away_team_impact'], ascending= [True, False]), x = 'away_team_impact', y = 'away_team', hue = 'division', dodge = False)
plt.show()
