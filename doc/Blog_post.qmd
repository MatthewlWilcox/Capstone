...
# Replace Month Variable String as a Number
...
# Create Date Variable in match_data so it is same format as betting_data
...
# Filter Data to only be before 2019
data = data[data['year']<= 2019]

# Remove unecessary Variables and rename columns
...
# Clean raw attendance and make it a float
...
# Make the Season Variable with 7/14 as the cut off date
season = []
...
# Make Mean Attendance, Standard deviation, and Z-Score Variables
...
# Remove the Scottish Second Division
total_data = total_data[total_data['division'] != 'SC2']


# Division Dictionary to Change League Names
...
# Make team names not snake case
...
#| echo: true
# Filter Dataset to only relavant variables 
...
#| warning: false
#| echo: true
# Filter data to only variables for analysis of time and date
time_df = total_data[[
...
# Group by count for the day of the week
...
# Group by day of the week and division
...
# Group by count for time of day
...
# Group by the count for each calendar day
...
# Group by the mean for each calendar day
...
```{python}
#| code-fold: false
# Group by the mean Attendance Z-score for the away team and league
...
```{=html}
<iframe width="700" height="700" src="https://awsa.wilcoxclan.com/"></iframe>
```
...
# Filter the data to only the top 3 teams per league
...
# Filter to only Premier League, Serie A, Bundesliga, Ligue 1, La Liga Primera
...
# Filter to only Premier League, Championship, League 1, and League 2
...
# Determine what result B365 favored
...
# Determine what result Bet&Win favored
...
# Determine what result William Hill favored
...
# Determine what result VC Bet favored
...
h_fav = []
attend_increase = []
# Create Boolean Variable on if Home team was favored to win
...
bet_bool = bet_bool.groupby(['home_fav', 'attend_increase']
).count().reset_index().rename(
    columns = {'home_fav':'Home Team Favorited',
     'attend_increase':'Increase In Attendance'})
```
...
# Dropping Missing data and unused variables
...
# Label Encode the data and make a key to reverse the encode
...
# For if Random Forest Model is imported
...
<iframe width="700" height="500" src="https://awsb.wilcoxclan.com/"></iframe>
...
# Determine if Predicted Attendnace was within a range
