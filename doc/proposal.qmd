...
Two data sets were used. The first was from Football-data.co.uk[see @Football_Betting_dataset], which contained the betting data for individual games. The second was scraped from Worldfootball.net[see @Football_match_data] and involved the individual match data. These will later be combined. Right now, as I am still scraping and combining the data sets, the dimensions of the data are unavailable. However, the variables that will be used are stated below:
...
3. How does the month affect the attendance of a match? Either will be a bar plot with all 12 months being the bars or a heat map of all the individual months. The data will be from all of the years, just the months, all grouped together and averaged out. 
...
6. How does the position in the table affect the attendance of a match? This one will be a bar graph where each bar is the position within the league, showing the average attendance for each position

7. How do promotion and relegation battles affect the attendance of a match? Here either it will just be a number or a bar graph of average attendance for teams in the promotion and relegation battles compared to not being in the battle.

8. How does the position in the table affect the attendance of the match over a season? This will be a time series graph of the entire season. Here the teams will be grouped into thirds to show teams that are on the top and bottom of the table and how that changes over the course of a season.

9. How does securing promotion or the league and securing relegation affect attendance compared to before securing their position? This will be a grouped bar graph with one group being promotion and the other being relegation. They will have two bars, one being the average attendance before securing, and the second would be the average attendance after securing their position.

10. How does the difference between positions affect the attendance of a match? This will be done in a histogram where the x-axis is the difference in league positions, and the y-axis is attendance. 

**Betting:**

11. How does attendance change when there is a heavy favorite vs. a closely betting team? 
...
14. How can all these factors be used to predict the attendance outcome of these games? Here I will build a model that uses factors of day/time, position in the table, and betting odds to predict what the attendance will be. 

15. Does attendance data and trends have changed after the covid 19 pandemic? Using the model seeing if it is still applicable with new data after being trained on pre-covid data. 
